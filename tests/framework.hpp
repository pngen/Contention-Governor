// Contention Governor test harness (dependency-free).
#ifndef CONTENTION_TEST_FRAMEWORK_HPP
#define CONTENTION_TEST_FRAMEWORK_HPP
...
inline void pass(const std::string& name) { std::printf("  [ok]   %s\n", name.c_str()); }
...
#define CONTENTION_TEST_IMPL \
  int main(int argc, char** argv) { return ::ctest::run_all(argc, argv); }
...
#define CHECK(cond) \
  do { if (!(cond)) { ::ctest::fail("CHECK failed: " #cond); } } while (0)

#define CHECK_EQ(a, b) \
  do { const auto& va_ = (a); const auto& vb_ = (b); \
       if (!(va_ == vb_)) { std::stringstream ss_; ss_ << "CHECK_EQ failed: " #a " == " #b " (" << va_ << " != " << vb_ << ")"; \
                            ::ctest::fail(ss_.str()); } } while (0)

#define REQUIRE(cond) \
  do { if (!(cond)) { ::ctest::fail("REQUIRE failed: " #cond); return; } } while (0)
